...
int main(int argc, char*argv[])
{
    ...
    sprintf(saveDirName, "/projects/bbtm/cli6/glassyDynamics/data/N%i/productionRuns/p%.3f/",numpts,p0);
...
//    voronoiModel->setCellPreferencesWithRandomAreas(p0,0.8,1.2);
    ...
    for(long long int ii = 0; ii < 2 * runTimesteps+2; ++ii)
        ...
        if (ii % spacingofDerivative == 0){
            ...
            derivativeCount++;
            if (derivativeCount%500 == 1){
                cout<<"Detivative steps: "<<derivativeCount-500<<"-"<<derivativeCount<<endl;
