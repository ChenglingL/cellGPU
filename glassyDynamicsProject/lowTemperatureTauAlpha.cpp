...
int main(int argc, char*argv[])
{
    ...
    sprintf(saveDirName, "/projects/bbtm/cli6/glassyDynamics/data/longtimeSimulation/N%i/",numpts);
    ...
    int lastOffsetSAC=1000000;
    int NwaitTauSAC=0;
    while(lastOffsetSAC < maximumWaitingTimesteps)
        {
        if(NwaitTauSAC>3){break;}
        offsetsSAC.push_back(lastOffsetSAC);
        cout << "save stress auto correlation function with an offset of " << lastOffsetSAC << endl;
        NwaitTauSAC++;
        lastOffsetSAC = NwaitTauSAC*5000000+1000000;
        }

    for(int ii = 0; ii < offsetsSAC.size(); ++ii)
        {
        sprintf(SACname,"%sSACtime_N%i_p%.4f_T%.8f_waitingTime%i_idx%i.nc",saveDirName,numpts,p0,T,ii*50000+10000,recordIndex);
