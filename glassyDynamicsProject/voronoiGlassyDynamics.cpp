...
int main(int argc, char*argv[])
{
    ...
    sprintf(saveDirName, "/projects/bbtm/cli6/glassyDynamics/data/N%i/",numpts);
    ...
    for(int ii = 0; ii < offsets.size(); ++ii)
        {
        sprintf(dataname,"%sglassyDynamics_N%i_p%.4f_T%.8f_waitingTime%.6f_idx%i.nc",saveDirName,numpts,p0,T,offsets[ii]*dt,recordIndex);
    ...
    while(lastOffsetSAC < maximumWaitingTimesteps)
        {
        if(NwaitTauSAC>5){break;}
        offsetsSAC.push_back(lastOffsetSAC);
        cout << "save derivatives with an offset of " << lastOffsetSAC << endl;
        NwaitTauSAC++;
        lastOffsetSAC = floor(NwaitTauSAC*10.0*tauEstimate/dt);
        }

    for(int ii = 0; ii < offsetsSAC.size(); ++ii)
        {
        sprintf(SACname,"%sSACtime_N%i_p%.4f_T%.8f_waitingTime%itimes%.2f_idx%i.nc",saveDirName,numpts,p0,T,ii,tauEstimate,recordIndex);
        ...
        lsacwriter.addDatabase(SACtime,acdat,offsetsSAC[ii],2000000);
        }
