...
/*!
This .cpp is to fill up the missing trajectories generated by
productionRuns. This .cpp load the glassydynamics.nc.
...
int main(int argc, char*argv[])
{
    ...
    sprintf(saveDirName, "/scratch/bbtm/cli6/glassyDynamics/data/N%i/missingTrajectory/p%.3f/",numpts,p0);
 
     
    sprintf(dataname,"%smissingTrajectory_N%i_p%.4f_T%.8f_idx%i.nc",saveDirName,numpts,p0,T,recordIndex);

    //check if the file is already existed and larger than 1kb
    std::filesystem::path filePath(dataname); // Replace with your file path
    ...
    //     // Check if the file size is larger than 1KB
    //     if (fileSize > 1024) {
    //         std::cout<< dataname << " exists and is larger than 1KB. Exiting..." << std::endl;
    //         return 0; // Exit the program
    //     } else {
    //         std::cout << dataname <<" exists but is not larger than 1KB." << std::endl;
    //     }
    // } else {
    //     std::cout << dataname << " does not exist." << std::endl;
    // }

    // Continue with other execution if needed
    std::cout << "Continuing with extended trajectory..." << std::endl;
...
//    voronoiModel->setCellPreferencesWithRandomAreas(p0,0.8,1.2);
    ...
    for(long long int ii = 0; ii < runTimesteps+10; ++ii)
        {
