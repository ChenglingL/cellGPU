...
/*!
THis is to test the modified nvt Nose-Hoover thermostats and record the energy stats. Here we use the old updater.
...
int main(int argc, char*argv[])
{
    ...
    while((c=getopt(argc,argv,"n:g:m:s:r:a:i:v:b:x:y:z:p:t:e:w:d:k:")) != -1)
        switch(c)
        {
            ...
            case 't': timeStep = atoi(optarg); break;
    ...
    long long int runTimesteps = floor(timeStep/dt); // get as many data points as possible for low T

    char saveDirName[256];
    char potentialEnergyname[256];
    char kineticEnergyname[256];

    sprintf(saveDirName, "/home/chengling/Research/Project/Cell/glassyDynamics/localTest/nvtTest/");

    sprintf(potentialEnergyname,"%stimePotentialEnergy_Old_N%i_p%.4f_T%.8f.nc",saveDirName,numpts,p0,T);
    shared_ptr<twoValuesDatabase> potentialEnergydat=make_shared<twoValuesDatabase>(potentialEnergyname,NcFile::Replace);
    sprintf(kineticEnergyname,"%stimeKineticEnergy_Old_N%i_p%.4f_T%.8f.nc",saveDirName,numpts,p0,T);
    shared_ptr<twoValuesDatabase> kineticEnergydat=make_shared<twoValuesDatabase>(kineticEnergyname,NcFile::Replace);
