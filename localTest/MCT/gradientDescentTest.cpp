...
/*!
This file is used to test how GD works fpr p0=3.85, if it returns false, return the conf
*/
...
int main(int argc, char*argv[])
{
    ...
    sprintf(inherentgDataname,"%sGDtest_success_time_N%i_p%.4f_T%.8f_idx%i.nc",savefolder,numpts,p0,T,recordIndex);
    ...
    for(int rec=fluidConfigurations.GetNumRecs()-10;rec<fluidConfigurations.GetNumRecs();rec++){
        ...
        cout<<"ready for initialize GD"<<endl;
        ...
        for (int ii = 0; ii < initSteps; ++ii)
            ...
            if(minimizeTime > 10)
            {
                ...
                char dataname[256];
                sprintf(dataname,"%sGDflaseConf_N%i_p%.3f_T%.8f_%i.nc",savefolder,numpts,p0,T,recordIndex);
            ...
            double energy = voronoiModel->Energy;
            printf("maxForce = %g, energy = %.8f\n",mf,energy);
            t3=clock();
