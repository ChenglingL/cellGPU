...
int main(int argc, char*argv[])
{
    ...
    for (int i=0;i<TNVT.size();i++){
        for (int idx=0;idx<10;idx++){
            ...
            std::vector<double> cageRelativeSISFdatNVT(fluidConfigurations.GetNumRecs());
            ...
            for(int rec=0;rec<fluidConfigurations.GetNumRecs();rec++){
                ...
                cageRelativeSISFdatNVT[rec] = dynFeat.computeCageRelativeSISF(voronoiModel->returnPositions());        
            };   
            //save the data
            //char overlapnameNVT[256];
            char cageRelativeSISFnameNVT[256];
            ...
            sprintf(cageRelativeSISFnameNVT,"/home/chengling/Research/Project/Cell/preTauAlpha/data/cageRelativeSISFNVTModified_N4096_p3.8000_T%.8f_waitingTime%.6f_idx%i.csv",TNVT[i],TimeNVT[i],idx);
            ...
            std::ofstream outFile2(cageRelativeSISFnameNVT);
            if (outFile2.is_open()) {
                for (int ii = 0; ii < cageRelativeSISFdatNVT.size(); ++ii) {
                    outFile2 << cageRelativeSISFdatNVT[ii];
                    if (ii != cageRelativeSISFdatNVT.size() - 1) {
                        outFile2 << ','; // Add a comma if it's not the last element
                    }
                }
                outFile2.close();
                std::cout << "Vector saved to " << cageRelativeSISFnameNVT << " successfully." << std::endl;
            } else {
                std::cerr << "Unable to open file." << std::endl;
            }
        }
