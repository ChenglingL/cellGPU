...
/*!
This is for CRoverlap and cage-relative SISF calculation for the data generated by taualpha.cpp for the missingTrajectories where the time steps recorded are the sasme.
Missingtrajectory stroes snapshots at the same intermidiate time steps
*/
...
int main(int argc, char*argv[])
{
    ...
    char CRSISFDataName[256];
    char CRoverlapDataName[256];
    char savefolder[256];
    char loadfolder[256];
    ...
    sprintf(loaddataname,"%smissingTrajectory_N%i_p%.4f_T%.8f_idx%i.nc",loadfolder,numpts,p0,T,recordIndex);
    sprintf(CRSISFDataName,"%stimeCRSISF_MissingTrajectory_N%i_p%.4f_T%.8f_waitingTime%.0f_idx%i.nc",savefolder,numpts,p0,T,waitingtime,recordIndex);
    sprintf(CRoverlapDataName,"%stimeCRoverlap_MissingTrajectory_N%i_p%.4f_T%.8f_waitingTime%.0f_idx%i.nc",savefolder,numpts,p0,T,waitingtime,recordIndex);
    ...
    shared_ptr<twoValuesDatabase> CRSISF=make_shared<twoValuesDatabase>(CRSISFDataName,NcFile::Replace);
    shared_ptr<twoValuesDatabase> CRoverlap=make_shared<twoValuesDatabase>(CRoverlapDataName,NcFile::Replace);
    ...
    for(int rec=0;rec<fluidConfigurations.GetNumRecs();rec++){
        ...
        CRoverlap->writeValues(time, dynFeat.computeCageRelativeOverlapFunction(voronoiModel->returnPositions()));        
        CRSISF->writeValues(time, dynFeat.computeCageRelativeSISF(voronoiModel->returnPositions(),ks));        
    };   
