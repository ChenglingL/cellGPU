...
int main(int argc, char*argv[])
{
    ...
    char DFDataName[256];
    char CRDFDataName[256];
    char savefolder[256];
    char DFsavefolder[256];
    char loadfolder[256];
    sprintf(savefolder,"/home/chengling/Research/Project/Cell/glassyDynamics/N%i/tauAlphaData/p%.3f/",numpts,p0);
    sprintf(DFsavefolder,"/home/chengling/Research/Project/Cell/glassyDynamics/N%i/displacementField/p%.3f/",numpts,p0);
    ...
    sprintf(DFDataName,"%sdisplacementField_N%i_p%.4f_T%.8f_id%i.nc",DFsavefolder,numpts,p0,T,recordIndex);
    sprintf(CRDFDataName,"%scageRelativeDisplacementField_N%i_p%.4f_T%.8f_id%i.nc",DFsavefolder,numpts,p0,T,recordIndex);
    ...
    shared_ptr<displacementModelDatabase> CRDFdat=make_shared<displacementModelDatabase>(numpts,CRDFDataName,NcFile::Replace);
    ...
    vector<double2> CRinputDat;
    CRinputDat = dynFeat.iPos;
    CRDFdat->writeState(CRinputDat,voronoiModel->currentTime,0);
    MSDdat->writeValues(voronoiModel->currentTime, 0.0);
    CRMSDdat->writeValues(voronoiModel->currentTime, 0.0);  
    ...
    for(int rec=1;rec<fluidConfigurations.GetNumRecs();rec++){
        ...
        CRinputDat = dynFeat.cageRelativeDisplacements;
        CRDFdat->writeState(CRinputDat,voronoiModel->currentTime);
