...
int main(int argc, char*argv[])
{
    ...
    while((c=getopt(argc,argv,"n:g:m:s:r:a:i:v:b:x:y:z:p:t:e:w:")) != -1)
        switch(c)
        {
            ...
            case 'i': initialSteps = atof(optarg); break;
    ...
    sprintf(saveDirName, "/home/chengling/Research/Project/Cell/glassyDynamics/systemSizeEffectGeq/");
    sprintf(simpleShearname,"%ssimpleShearDerivatives_N%i_p%.4f_T%.8f_idx%i.nc",saveDirName,numpts,p0,T,recordIndex);
    shared_ptr<derivativeModelDatabase> simplesheardat=make_shared<derivativeModelDatabase>(numpts, simpleShearname,NcFile::Replace);
...
//    voronoiModel->setCellPreferencesWithRandomAreas(p0,0.8,1.2);
    ...
    for(long long int ii = 0; ii < runTimesteps+2; ++ii)
        ...
        if (ii % spacingofDerivative == 0){
            ...
            simplesheardat->writeState(voronoiModel);
            }
