...
/*!
This is for true MFD for the data generated by taualpha.cpp. tHIS IS SPEFCIFICALLY FOR PRODUCTION RUNS in delta.
*/
...
int main(int argc, char*argv[])
{
    ...
    sprintf(CRMFDDataName,"%stimeTrueCRMFD_N%i_p%.4f_T%.8f_waitingTime%.0f_idx%i.nc",savefolder,numpts,p0,T,waitingtime,recordIndex);
    ...
    cout << "save record in " << CRMSDDataName << endl;
    shared_ptr<twoValuesDatabase> CRMSDdat=make_shared<twoValuesDatabase>(CRMSDDataName,NcFile::Replace);
    shared_ptr<twoValuesDatabase> CRMFDdat=make_shared<twoValuesDatabase>(CRMFDDataName,NcFile::Replace);
    ...
    for(int rec=1;rec<fluidConfigurations.GetNumRecs();rec++){
        ...
        CRMSDdat->writeValues(time - initialTime, dynFeat.computeCageRelativeMSD(voronoiModel->returnPositions(),voronoiModelprevious->returnPositions(),previousWhichBox));        
        CRMFDdat->writeValues(time - initialTime, dynFeat.computeCageRelativeMFD(voronoiModel->returnPositions(),voronoiModelprevious->returnPositions(),previousWhichBoxCR));        
