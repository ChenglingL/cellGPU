...
int main(int argc, char*argv[])
{
    ...
    sprintf(saveTraname,"%strajectory_N%i_p%.4f_T%.8f_waitingTime%.0f_idx%i.nc",loadfolder,numpts,p0,T,waitingtime,recordIndex);
    
    sprintf(saveDataName,"%sneighborOverlap_trajectoy_N%i_p%.4f_T%.8f_waitingTime%.0f_idx%i.nc",savefolder,numpts,p0,T,waitingtime,recordIndex);
    ...
    shared_ptr<trajectoryModelDatabase> trajetorydat=make_shared<trajectoryModelDatabase>(numpts,saveTraname,NcFile::Replace);
    ...
    for(int rec=0;rec<fluidConfigurations.GetNumRecs();rec++){
        ...
        voronoiModel->enforceTopology();
        trajetorydat->writeState(voronoiModel);
