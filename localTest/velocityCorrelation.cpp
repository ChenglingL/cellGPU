...
int main(int argc, char*argv[])
{
    ...
    sprintf(saveDirName, "/home/chengling/Research/Project/Cell/glassyDynamics/N%i/velocityCorrelation/",numpts);
    ...
    sprintf(dataname,"%svelocityCorrelation_N%i_p%.4f_T%.8f_dt0.01_idx%i.nc",saveDirName,numpts,p0,T,recordIndex);
    std::ofstream file(dataname); // Open CSV file
        if (!file) {
...
//    voronoiModel->setCellPreferencesWithRandomAreas(p0,0.8,1.2);
    ...
    for(long long int ii = 0; ii < runTimesteps+2; ++ii)
        {
        double velocityCor= dynFeat.computeVelocityCorrelation(voronoiModel->returnVelocities());
        file << velocityCor << "\n";  // Write only energy

    // Optional: Flush the file to ensure data is written immediately
        file.flush();
        sim->performTimestep();
        };
