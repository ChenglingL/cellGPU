...
for recordIdx in ${records[@]}; do
    for i in ${!tauEstimate[@]}; do
        ...
        /home/chengling/Research/Project/Cell/AnalyticalG/cellGPU/localTest/executable/chi4.out -n ${number} -p ${p} -v ${temp} -t ${tauEst} -i ${eqWaitMultiple} -m ${nRelaxations} -r ${recordIdx} -g -1
