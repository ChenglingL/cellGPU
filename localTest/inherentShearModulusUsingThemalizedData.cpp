...
int main(int argc, char*argv[])
{
    ...
    sprintf(inherentgDataname,"%sinherentgAffineG_N%i_p%.3f_KA%.4f_T0.02.nc",saveDirName,numpts,p0,KA);
    char eigenValueDataname[256];
    sprintf(eigenValueDataname,"%seigenValue_N%i_p%.3f_KA%.4f_T0.02.nc",saveDirName,numpts,p0,KA);
    ...
    for (int idx = 0; idx < Nconfigurations; idx++)
    {
        ...
        cout << "reading record " << idx << " from " << loadDatabaseName << endl;
        if(idx <fluidConfigurations.GetNumRecs())
            {
            //cout<<"Start to load data!"<<endl;
            fluidConfigurations.readState(spv,idx,true);
        ...
        //spv->setBidisperseCellPreferences(p0,v0,0.5);
