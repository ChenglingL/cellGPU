...
/*!
This .cpp is to get the the stress trajectory generated by
productionRuns and stress trajectory from contribution A and P. This .cpp load the glassydynamics.nc
*/
...
int main(int argc, char*argv[])
{
    ...
    sprintf(saveDirName, "/home/chengling/Research/Project/Cell/glassyDynamics/N%i/productionRuns/p%.3f/stressTrajectory/",numpts,p0);
 
     
    sprintf(datanamePeri,"%sstressTrajectory_perimeter_N%i_p%.4f_T%.8f_idx%i.nc",saveDirName,numpts,p0,T,recordIndex);
    sprintf(datanameArea,"%sstressTrajectory_area_N%i_p%.4f_T%.8f_idx%i.nc",saveDirName,numpts,p0,T,recordIndex);
...
//    voronoiModel->setCellPreferencesWithRandomAreas(p0,0.8,1.2);
    ...
    for(long long int ii = 0; ii < 100; ++ii)
    {
        sim->performTimestep();
    };
    ...
    for(long long int ii = 0; ii < runTimesteps; ++ii)
        ...
        SACtime->writeValues(ii,sigmaPeri);
        SACtimeArea->writeValues(ii,sigmaArea);
