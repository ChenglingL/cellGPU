...
/*!
This is to compute the potential energy for the data generated by taualpha.cpp. tHIS IS SPEFCIFICALLY FOR PRODUCTION RUNS in delta.
*/
...
int main(int argc, char*argv[])
{
    ...
    char savefolder[256];
    char loadfolder[256];
    sprintf(savefolder,"/home/chengling/Research/Project/Cell/glassyDynamics/N%i/productionRuns/p%.3f/energy/",numpts,p0);
    ...
    if (!std::filesystem::exists(savefolder)) {
        ...
        if (std::filesystem::create_directory(savefolder)) {
            std::cout << "Folder created successfully: " << savefolder << std::endl;
        } else {
            std::cerr << "Failed to create folder: " << savefolder << std::endl;
        }
    } else {
        std::cout << "Folder already exists: " << savefolder << std::endl;
    }
    ...
    sprintf(saveDataName,"%stimePotentialEnergy_N%i_p%.4f_T%.8f_waitingTime%.0f_idx%i.nc",savefolder,numpts,p0,T,waitingtime,recordIndex);
    ...
    shared_ptr<twoValuesDatabase> energydat=make_shared<twoValuesDatabase>(saveDataName,NcFile::Replace);
    ...
    for(int rec=fluidConfigurations.GetNumRecs()-11;rec<fluidConfigurations.GetNumRecs();rec++){
