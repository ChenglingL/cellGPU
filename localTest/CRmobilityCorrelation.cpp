...
/*!
This is for cage-relative mobilotyCorrelation calculation
*/
...
int main(int argc, char*argv[])
{
    ...
    sprintf(saveDataName,"%sCRmobilityCorrelation_N%i_p%.4f_T%.8f_%i.nc",saveDirName,numpts,p0,T,id);
    //cout<<loaddataname<<endl;
    shared_ptr<twoValuesDatabase> mobilotyCorrelation=make_shared<twoValuesDatabase>(saveDataName,NcFile::Replace);
    ...
    for(int rec=0;rec<fluidConfigurations.GetNumRecs();rec++){
        ...
        mobilotyCorrelation->writeValues(voronoiModel->currentTime,dynFeat.computeCageRelativeMobilityCorrelation(voronoiModel->returnPositions()));        
    }
