...
/*!
This file is used to test the calculation of inherent state g to reproduce "No unjamming transition in a Voronoi model of biological tissue"
*/
...
int main(int argc, char*argv[])
{
    ...
    sprintf(inherentgDataname,"%sinherentgRightAffineOldPaper_Bi_N%i_p%.3f_KA%f.4.nc",saveDirName,numpts,p0,KA);
    ...
    for (int idx = 0; idx < Nconfigurations; idx++)
    {
        ...
        double nonaffineWrong = 0.0;
        vector<double2> derivative;
        spv->getd2EdgammadrOldPaper(derivative);
        vector<double2> derivativewrong;
        spv->getd2EdgammadrOldPaperWrong(derivativewrong);
        ...
        for (int i = 0; i < D.eigenvalues.size(); i++)
        {
            ...
            if (D.eigenvalues[i]>thresh)
            {
                ...
                double localnonAWrong = 0.0;
                for (int ii = 0; ii < numpts; ++ii)
                {
                    localnonA += derivative[ii].x * localEigenVector[2*ii];
                    localnonA += derivative[ii].y * localEigenVector[2*ii+1];
                    localnonAWrong += derivativewrong[ii].x * localEigenVector[2*ii];
                    localnonAWrong += derivativewrong[ii].y * localEigenVector[2*ii+1];
                };
                nonaffine += 1.0/D.eigenvalues[i] * localnonA * localnonA;
                nonaffineWrong += 1.0/D.eigenvalues[i] * localnonAWrong * localnonAWrong;
            }

        }
        double d2edg2=spv->getd2EdgammadgammaOldPaper();
        double g = (d2edg2 - nonaffine)/numpts;
        double gwrong = (d2edg2 - nonaffineWrong)/numpts;
        cout<<"the affine shear modulus is "<<d2edg2/numpts<<" and the non-affine shear modulus is "<<g<<endl;
        cout<<"the affine shear modulus is "<<d2edg2/numpts<<" and the wrong non-affine shear modulus is "<<gwrong<<endl;
