    ...
    for (int i=0;i<TNVT.size();i++){
        for (int idx=0;idx<10;idx++){
            for (int nnn=0;nnn<3;nnn++){
                ...
                sprintf(MSDnameNVT,"/home/chengling/Research/Project/Cell/AnalyticalG/data/N%i/nvt_MSD_N%i_p%.3f_T%.8f_%i.csv",Ncells[nnn],Ncells[nnn],p0,TNVT[i],idx);
                sprintf(SISFnameNVT,"/home/chengling/Research/Project/Cell/AnalyticalG/data/N%i/nvt_SISF_N%i_p%.3f_T%.8f_%i.csv",Ncells[nnn],Ncells[nnn],p0,TNVT[i],idx);
                std::ofstream outFile1(MSDnameNVT);
                if (outFile1.is_open()) {
                    for (int ii = 0; ii < MSDdatNVT.size(); ++ii) {
                        outFile1 << MSDdatNVT[ii];
