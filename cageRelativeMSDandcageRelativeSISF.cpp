    ...
    for (int i=0;i<TNVT.size();i++){
        for (int idx=0;idx<10;idx++){
            for (int nnn=0;nnn<3;nnn++){
                ...
                sprintf(cageRelativeMSDnameNVT,"/home/chengling/Research/Project/Cell/AnalyticalG/data/N%i/nvt_CRMSD_N%i_p%.3f_T%.8f_%i.csv",Ncells[nnn],Ncells[nnn],p0,TNVT[i],idx);
                sprintf(cageRelativeSISFnameNVT,"/home/chengling/Research/Project/Cell/AnalyticalG/data/N%i/nvt_CRSISF_N%i_p%.3f_T%.8f_%i.csv",Ncells[nnn],Ncells[nnn],p0,TNVT[i],idx);
