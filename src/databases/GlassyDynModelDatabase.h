#ifndef DATABASE_glassyD_H
#define DATABASE_glassyD_H

#include "voronoiQuadraticEnergy.h"
#include "dynamicalFeatures.h"
#include "DatabaseNetCDF.h"

/*! \file GlassyDynModelDatabase.h */
...
*/
class GlassyDynModelDatabase : public BaseDatabaseNetCDF
{
private:
    ...
    NcVar *d2EdgammadgammaVar, *sigmaVar, *energyVar, *BoxMatrixVar; //!<NcVars we'll use *posVar,
    int Current;    //!< keeps track of the current record when in write mode


public:
    GlassyDynModelDatabase(int np, string fn="temp.nc", NcFile::FileMode mode=NcFile::ReadOnly);
    ~GlassyDynModelDatabase(){File.close();};
