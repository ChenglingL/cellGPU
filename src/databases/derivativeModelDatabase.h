#ifndef DATABASE_derivative_H
#define DATABASE_derivative_H

#include "voronoiQuadraticEnergy.h"
#include "dynamicalFeatures.h"
#include "DatabaseNetCDF.h"

/*! \file derivativeModelDatabase.h */
...
*/
class derivativeModelDatabase : public BaseDatabaseNetCDF
{
private:
    ...
    NcVar *d2EdgammadgammaVar,*sigmaiVar; //!<NcVars we'll use. The additional data is the current A and O in cell GPU
    int Current;    //!< keeps track of the current record when in write mode


public:
    derivativeModelDatabase(int np, string fn="temp.nc", NcFile::FileMode mode=NcFile::ReadOnly);
    ~derivativeModelDatabase(){File.close();};
